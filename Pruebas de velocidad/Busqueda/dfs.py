...
def generar_grafo(n, promedio_vecinos=5):
    ...
    # Convertir conjuntos a listas para DFS
    return {nodo: list(vecinos) for nodo, vecinos in grafo.items()}

# Función de búsqueda en profundidad en un grafo (iterativa para evitar recursión profunda)
def dfs_search(grafo, start, target):
    if start == target:
        return start
    stack = deque([start])  # Usar deque como pila para eficiencia
    visited = {start}
    
    while stack:
        node = stack.pop()  # Pop desde el final (pila)
...
for i in range(15):
    ...
    start_time = time.time()
    result = dfs_search(grafo, 0, target)
